...
import { BellIcon, PlusIcon, TrashIcon, ArrowTrendingUpIcon, ArrowTrendingDownIcon, CurrencyDollarIcon, ExclamationTriangleIcon, SparklesIcon } from '@heroicons/react/24/outline';
  ...
  const loadAlerts = async () => {
    try {
      ...
      if (currentUser) {
        ...
        console.log('No current user, using mock alerts');
        setAlerts([
          {
            id: 1,
            symbol: 'BTC',
  ...
  const addAlert = async () => {
    try {
      ...
      if (currentUser) {
        ...
        setAlerts([...alerts, { ...alertData, id: Date.now(), triggered: false }]);
      }
    ...
    if (totalValue > 100000) {
      ...
      if (!alerts.some(alert => alert.type === 'portfolio' && alert.condition === 'below' && alert.threshold === portfolioDropThreshold)) {
        autoAlerts.push({
          ...
          type: 'portfolio',
          message: `Portfolio dropped below 90% of peak value (${portfolioDropThreshold.toLocaleString()})`,
          triggered: false,
          autoGenerated: true
        });
      }
      ...
      if (!alerts.some(alert => alert.type === 'portfolio' && alert.condition === 'above' && alert.threshold === portfolioGainThreshold)) {
        autoAlerts.push({
          ...
          type: 'portfolio',
          message: `Portfolio increased by 10% - consider taking profits (${portfolioGainThreshold.toLocaleString()})`,
          triggered: false,
          autoGenerated: true
        });
      }
      ...
      if (value > 25000) {
        ...
        if (!alerts.some(alert => alert.symbol === asset.symbol && alert.condition === 'below' && Math.abs(alert.threshold - dropThreshold) < 1)) {
          autoAlerts.push({
            ...
            type: 'price',
            message: `${asset.symbol} dropped 10% - consider buying more (${dropThreshold.toLocaleString()})`,
            triggered: false,
            autoGenerated: true
          });
        }
        ...
        if (!alerts.some(alert => alert.symbol === asset.symbol && alert.condition === 'above' && Math.abs(alert.threshold - gainThreshold) < 1)) {
          autoAlerts.push({
            ...
            type: 'price',
            message: `${asset.symbol} up 15% - consider taking profits (${gainThreshold.toLocaleString()})`,
            triggered: false,
            autoGenerated: true
          });
        }
      ...
      if (Math.abs(asset.change_percentage_24h || 0) > 5) {
        ...
        if (!alerts.some(alert => alert.symbol === asset.symbol && alert.condition === 'below' && Math.abs(alert.threshold - volatilityThreshold) < 1)) {
          autoAlerts.push({
            ...
            type: 'price',
            message: `${asset.symbol} high volatility - watch for further decline (${volatilityThreshold.toLocaleString()})`,
            triggered: false,
            autoGenerated: true
          });
        }
    ...
    if (portfolioData.risk_level === 'High') {
      ...
      if (!alerts.some(alert => alert.type === 'portfolio' && alert.condition === 'below' && alert.threshold === riskDropThreshold)) {
        autoAlerts.push({
          ...
          type: 'portfolio',
          message: `High risk portfolio dropped 20% - consider reducing exposure (${riskDropThreshold.toLocaleString()})`,
          triggered: false,
          autoGenerated: true
        });
      }
      ...
      if (!alerts.some(alert => alert.type === 'portfolio' && alert.condition === 'above' && alert.threshold === riskGainThreshold)) {
        autoAlerts.push({
          ...
          type: 'portfolio',
          message: `High risk portfolio up 5% - review risk management (${riskGainThreshold.toLocaleString()})`,
          triggered: false,
          autoGenerated: true
        });
      }
